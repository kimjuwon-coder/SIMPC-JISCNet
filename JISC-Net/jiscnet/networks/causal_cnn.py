...
class CausalConvolutionBlock(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, dilation,
        ...
        chomp1 = Chomp1d(padding)  #zero padding으로 인해 생긴 불필요한 패딩 제거 
    ...
    def forward(self, x):
        #x = x.to(device)  #torch.Size([1, 3, 60]) ->
        out_causal = self.causal(x) #torch.Size([1, 32, 60]) -> 
...
class CausalCNNEncoder(torch.nn.Module):
    ...
    def __init__(self, in_channels, channels, depth, reduced_size,
        ...
        reduce_size = torch.nn.AdaptiveMaxPool1d(1) #torch.Size([N, 64, 60]) -> torch.Size([N, 64, 1]) # 맥스 값만 남겨놓음 
        squeeze = SqueezeChannels()  #torch.Size([N, 64, 1]) -> torch.Size([N, 64]) #시간 dimension 없애기 
